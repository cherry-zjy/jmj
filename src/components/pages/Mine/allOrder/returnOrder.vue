<template>
  <div class="root" :style="'min-height:'+allHeight +'px'">
    ...
    <div class="goods-info">
      <div class="list">
        ...
        <div class="order-good">
          <img class="goods-img" :src="orderInfo.Image">
          <div class="goods-title">
            <p>{{orderInfo.Name}}</p>
            <span>￥{{orderInfo.Price}}</span>
          </div>
          <div class="goods-num">
            <span>X {{orderInfo.Count}}</span>
          </div>
        </div>
        <div class="pay">
          ...
          <div class="right">
            <img
              ...
              src="../../../../../static/images/alipay_big@2x.png"
              alt
            >
            <img
              class="img1"
              v-else-if="orderInfo.PayType == 1"
    ...
    <div class="area">
      <!-- <p>1231</p> -->
      <div class="text-area">
        <textarea cols="30" rows="8" v-model="content" placeholder="退货理由：为什么呢？"></textarea>
      </div>
    </div>
    <div class="footer-btn" @click="cancel()">商品退货</div>
    <!-- 确认 -->
    <mt-popup v-model="popupVisible" popup-transition="popup-fade" class="orderPsw-popup">
      <div>
        <h2>审核通过后商品才能退货</h2>
        <div class="footer">
          <p @click="popupVisible = false">手滑了</p>
          <p @click="CancellationOfOrder()">退货</p>
        </div>
      </div>
    </mt-popup>
  </div>
...
export default {
  ...
  methods: {
    ...
    CancellationOfOrder () {
      this.$http
        .get('/api/User/RefundSomeGood', {
          Token: this.$store.state.token || localStorage.getItem('token'),
          ordProdID: this.orderInfo.OrdProdID,
          reason: this.content
        })
        .then(res => {
  ...
  mounted () {
    ...
    this.orderInfo = JSON.parse(localStorage.getItem('goodsIndo'))
  }
}
</script>
