...
export default {
  ...
  methods: {
    ...
    getInfoList (p, t) {
      this.$http.get('/api/User/GetUserConcessionList',
        {
          state: t,
          pageIndex: p,
          pageSize: 10
        ...
        .then(res => {
          this.infoList = res.Result.userconlist
    ...
    choiceWallet (info, refAdd) {
      if (this.isChoice) {
        console.log(this.$refs.couponRed)
        // 订单选择优惠券 如果是订单进来的选择存起来
        localStorage.setItem('choiceCoupon', JSON.stringify(info))
        this.$refs.couponRed.forEach(item => {
          item.id = ''
        })
        this.$refs.couponRedKong.id = ''
        this.$refs.couponRed[refAdd].id = 'isChoiveCoupon'
        setTimeout(() => {
          this.$router.go(-1)
        }, 0)
      }
    ...
    choiceWalletKong () {
      localStorage.setItem('choiceCoupon', JSON.stringify({ID: '-1'}))
      this.$refs.couponRed.forEach(item => {
        item.id = ''
      })
