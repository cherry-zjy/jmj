  ...
  <div class="root" :style="'min-height:'+htmlH +'px'">
    ...
    <div class="top_address" @click="goAddress()">
      ...
      <div class="left user-add">
        ...
        <p
          v-if="idHaveAdd"
        >收货地址：{{addressInfo.Province}}{{addressInfo.City}}{{addressInfo.Region}}{{addressInfo.Address}}</p>
    ...
    <div v-for="(l,i) in orderInfo.ProdList" :key="i">
      <!-- 商品信息 -->
      <div class="overseas-list" v-if="urlType == 2">
      ...
      <div class="list-info">
        <div class="list" v-if="urlType == 0">
          <div class="left">{{orderInfo.SelectSpec}}</div>
          <div class="right">
            <span>预售中</span>
            <img src="../../../../../static/images/enter@2x.png" alt>
          </div>
        ...
        <div class="list" @click="choicePayType()">
          ...
          <div class="right">
            <img
              class="img1"
              v-if="payTypeNum == 0"
              src="../../../../../static/images/wechat_big@2x.png"
              alt
            >
    ...
    <mt-popup v-model="popupVisible" popup-transition="popup-fade" class="orderPsw-popup">
      <div>
        ...
        <div class="footer-tip">
          ...
          <p>
            <button @click="ConfirmationOfReceipt($event)">确定</button>
          </p>
        </div>
      </div>
    </mt-popup>
...
export default {
  ...
  methods: {
    ...
    getOrderInfo1 () {
      this.$http
        .get('/api/APP_Product/GenerateOrderData', {
          orderID: this.urlID
        })
        .then(res => {
          ...
          if (localStorage.getItem('choiceAddress')) {
            this.addressInfo = JSON.parse(
              localStorage.getItem('choiceAddress')
            )
          } else {
            this.addressInfo = res.Result
          }
          if (localStorage.getItem('choiceCoupon')) {
            this.choiceCoupon = JSON.parse(
              localStorage.getItem('choiceCoupon')
            )
          }
    ...
    getOrderInfo () {
      this.$http
        ...
        .then(res => {
          ...
          if (this.urlType === 2) {
            if (!localStorage.getItem('choiceAuthor') && res.Result.ProdList[0].ProductAuthen === '') {
              Toast('请先身份认证，否则无法使用优惠')
            }
          }
          if (
            localStorage.getItem('choiceRed') ||
            localStorage.getItem('choiceCoupon') ||
            localStorage.getItem('choiceAddress') ||
            localStorage.getItem('choiceAuthor')
          ) {
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (res.Result.res === '支付成功') {
            Toast('支付成功')
            setTimeout(() => {
              this.$router.push('/mine/orderlist/2')
            }, 1000)
          } else {
    ...
    EditOrderPordISHB (ut) {
      if (this.addressInfo.Name) {
        this.$http
          .post('/api/APP_Product/EditOrderPord', {
            orderID: this.urlID,
            ...
            RealName:
              ut === 2
                ? JSON.parse(localStorage.getItem('choiceAuthor')).Name
                : -1
          })
          .then(res => {
  ...
  mounted () {
    ...
    this.getOrderInfo()
    // 判断当前是否存在抵用券红包的选择，如果存在，默认去修改一次订单数据。
  }
}
</script>
