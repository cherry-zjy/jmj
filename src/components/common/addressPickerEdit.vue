    ...
    <div class="page-content">
      ...
      <mt-popup v-model="areaVisible" class="area-class" position="bottom">
        <p>
          ...
          <span class="btn" @click="sub()">确定</span>
        </p>
...
export default {
  ...
  data () {
    // 编辑时设置默认选中的值
    let city = data[province.indexOf(this.address.split(',')[0])].children.map(
      res => {
        return res.name
      }
    )
    // 编辑时设置默认选中的值
    let area = data[province.indexOf(this.address.split(',')[0])].children[city.indexOf(this.address.split(',')[1])].children.map(res => {
      return res
    })
    return {
      areaVisible: false,
      ...
      slots: [
        {
          ...
          className: 'slot1',
          textAlign: 'center',
          defaultIndex: province.indexOf(this.address.split(',')[0]),
          value: this.address.split(',')[0]
        },
        {
          divider: true,
          content: '-',
        ...
        },
        {
          ...
          className: 'slot3',
          textAlign: 'center',
          defaultIndex: city.indexOf(this.address.split(',')[1]),
          value: this.address.split(',')[1]
        },
        {
          divider: true,
          content: '-',
        ...
        },
        {
          ...
          className: 'slot5',
          textAlign: 'center',
          defaultIndex: area.indexOf(this.address.split(',')[2]),
          value: this.address.split(',')[2]
        }
      ]
    }
  },
  ...
  methods: {
    handlerArea () {
      ...
      if (this.areaString === '省,市,区') {
        this.areaString = province[0] + ',' + city[0] + ',' + area[0]
      }
    },
